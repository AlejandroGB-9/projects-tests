...
class client :
    ...
    @staticmethod
    def escuchar(alias, window):
        data = b''
        client._socket.listen()
        while True:
            connection, client_address = client._socket.accept()    
            #se recibe un número 'count' o 'id' o una peticion, 'SEND_MESSAGE' o 'SEND_MESS_ACK'
            petition = client.readString(connection)
            match petition:
                case "SEND_MESSAGE":
                    id_c = client.readString(connection)
                    alias = client.readString(connection)
                    mensaje = client.readString(connection)
                    window['_SERVER_'].print("s> MESSAGE" + id_c + "FROM " + alias + "\n\t" + mensaje + "\n\tEND")
                    window['_CLIENT_'].print("c> MESSAGE" + id_c + "FROM " + alias + "\n\t" + mensaje + "\n\tEND")      
                case "SEND_MESS_ACK":
                    id_c = client.readString(connection)
                    window['_SERVER_'].print("s> SEND MESSAGE ACK " + id_c + " OK")
    ...
    def  register(user, window):
        ...
        answer = ''
        try:
            sock_s.sendall(petition.encode())
            sock_s.sendall(name.encode())
            sock_s.sendall(alias.encode())
            sock_s.sendall(date.encode())
            while True:
                msg = sock_s.recv(1)
                if (msg == b'\0'):
                    break
    ...
    def  unregister(user, window):
        ...
        server_address = (client._server, int(client._port))
        sock_s.connect(server_address)
        petition = "UNREGISTER\0"
        user = user +'\0'
        answer = ''
        try:
            sock_s.sendall(petition.encode())
            sock_s.sendall(user.encode())
            answer = client.readString(sock_s)
        finally:
            match answer:
                case "0":
    ...
    def  connect(user, window):
        ...
        client_address = (client._server, 0)
        client._socket.bind(client_address)
        client_port = client._socket.getsockname()[1]
        petition = "CONNECT\0"
        user = user + '\0'
        port = str(client_port) + '\0'
        answer=''

        try:
            sock_s.sendall(petition.encode())
            sock_s.sendall(user.encode())
            sock_s.sendall(port.encode())
            multi_thread = threading.Thread(target = client.escuchar,name='Daemon',args=(client._alias, window))
            multi_thread.start()
            answer = client.readString(sock_s)
        finally:
            match answer:
    ...
    def  send(user, message, window):
        ...
        try:
            ...
            sock_s.sendall(petition.encode())
            sock_s.sendall(alias.encode())
            sock_s.sendall(alias_dest.encode())
            sock_s.sendall(message.encode())   
            answer = client.readString(sock_s)
            id = client.readString(sock_s)
    ...
    def  connectedUsers(window):
        ...
        try:
            ...
            sock_s.sendall(alias.encode())
            answer = client.readString(sock_s)
            count = client.readString(sock_s)
            users = ''
            for i in range(int(count)):
                if i == 0:
                    users = client.readString(sock_s)
                else:
                    users = users + ", " + client.readString(sock_s)
        finally:
            match answer:
                case "0":
                    window['_SERVER_'].print("s> CONNECTED USERS " + count + " OK " + users)
                case "1":
                    window['_SERVER_'].print("s> CONNECTEDUSERS FAIL/ USER IS NOT CONNECTED")
                case "2":
                    window['_SERVER_'].print("s> CONNECTEDUSERS FAIL")
            sock_s.close()
        return client.RC.ERROR
